...
class UAnimMontage; 
class AItemBase;
class ABaseWeaponTemplate;
class USpotLightComponent;
struct FInputActionInstance;

DECLARE_MULTICAST_DELEGATE_OneParam(FOnTargetItemChanged, const FString&);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnItemAcquired, const FString&);
DECLARE_MULTICAST_DELEGATE_OneParam(FOnInteractionProgressChanged, float);
...
UCLASS()
class BREAKER_API ABreakerCharacter : public ACharacter
{
	GENERATED_BODY()
...
public:
	...
	ABaseWeaponTemplate* PrimaryWeapon;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Player|Combat")
	ABaseWeaponTemplate* SecondaryWeapon;

	UPROPERTY(BlueprintReadOnly, Category = "Player|Combat")
	ABaseWeaponTemplate* CurrentWeapon;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Player|Combat|Animation")
	TObjectPtr<UAnimMontage> AMHitReactionUnarmed;
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Player|Combat|Animation")
	TObjectPtr<UAnimMontage> AMHitReactionArmed;
	...
	FOnTargetItemChanged OnTargetItemChanged;
	...
	void InternalAttachWeapon(ABaseWeaponTemplate* Weapon, FName SocketName, const FTransform& Offset);
