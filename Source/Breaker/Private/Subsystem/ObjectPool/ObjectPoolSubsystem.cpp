...
#include "Subsystem/ObjectPool/PooledObject.h"
...
void UObjectPoolSubsystem::OnWorldInitialized(UWorld* World, const UWorld::InitializationValues IValues)
{
	...
	if (World->IsGameWorld())
	{
		...
		PoolAnchor = GetWorld()->SpawnActor(AActor::StaticClass(), &FVector::ZeroVector, &FRotator::ZeroRotator, SpawnParams);

		if (IsValid(PoolAnchor))
		{
			if (USceneComponent* PoolAnchorRootComponent = NewObject<USceneComponent>(PoolAnchor, TEXT("Root")))
			{
				PoolAnchorRootComponent->RegisterComponent();
				PoolAnchor->SetRootComponent(PoolAnchorRootComponent);
			}
		}
	}
}
